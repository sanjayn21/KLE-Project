...
# Add security_auditor directory to path for imports
current_dir = Path(__file__).parent
security_auditor_dir = current_dir / "security_auditor"
sys.path.insert(0, str(security_auditor_dir))
...
def test_nvd_api():
    ...
    print("[TEST] Testing NVD API Client...")
    
    client = NVDClient()
    
    # Test API call
    print("\n[API] Making test API call...")
    vulnerabilities = client.search_cves_by_keyword("openssl", limit=3)
    
    print(f"[SUCCESS] Found {len(vulnerabilities)} vulnerabilities for 'openssl'")
...
def test_ml_risk_analyzer():
    ...
    print("\n[ML] Testing ML Vulnerability Risk Analyzer...")
    ...
    print(f"[SUCCESS] ML Risk Score for sample vulnerability: {risk_score}")
...
def test_vulnerability_scanner():
    ...
    print("\n[SCANNER] Testing Enhanced Vulnerability Scanner...")
    
    scanner = VulnerabilityScanner()
    
    # Run a limited scan
    print("[SCAN] Running vulnerability scan...")
    ...
    if results['top_vulnerabilities']:
        print(f"\n[TOP] Top 3 Most Risky Vulnerabilities:")
...
def test_api_monitor():
    ...
    print(f"[SUCCESS] Monitor Summary:")
    ...
    print(f"[HEALTH] API Health Status: {health['health_status']}")
...
def main():
    """Run all tests"""
    print("Enhanced NVD API and ML Vulnerability Scoring Test")
    print("="*60)
